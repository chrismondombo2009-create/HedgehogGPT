/**
 * HedgehogGPT — FCA Utilities
 *
 * Contains parseMentions() — a multi-format Facebook mention parser.
 * Supports: prng (legacy), tags_metadata, profile_xmd.
 * All commands using Object.keys(event.mentions) continue to work unchanged.
 */
...
/**
 * Parses mentions from a raw Facebook delta object into a normalised map.
 *
 * Facebook has changed the mention payload format over time.
 * We try every known format in order and return the first one that yields data.
 *
 * Supported formats:
 *   A/B — legacy `prng` field: JSON string of [{i, o, l}]
 *   C   — `tags_metadata` field: array of {id, offset, length}
 *   D   — `profile_xmd` field: array of {id, offset, length}
 *
 * @param {object|undefined} deltaData  — m.delta.data object (may be undefined)
 * @param {string}           body       — message body text
 * @returns {{ [uid: string]: string }} — map of uid → tagged text
 */
function parseMentions(deltaData, body) {
  body = body || "";

  // Format A/B: legacy prng field
  try {
    const raw = deltaData && deltaData.prng;
    if (raw) {
  ...
  } catch (_) { /* prng absent or malformed — try next format */ }

  // Format C: tags_metadata field
  try {
    const raw = deltaData && deltaData.tags_metadata;
    if (raw) {
  ...
  } catch (_) { /* tags_metadata absent or malformed — try next format */ }

  // Format D: profile_xmd field
  try {
    const raw = deltaData && deltaData.profile_xmd;
    if (raw) {
      const parsed = typeof raw === "string" ? JSON.parse(raw) : raw;
      if (Array.isArray(parsed) && parsed.length > 0) {
        const out = {};
  ...
  } catch (_) { /* profile_xmd absent or malformed */ }

  // No mention data found in any known format
  return {};
}
...
function formatDeltaMessage(m) {
  ...
  return {
    ...
    // Use our multi-format parser instead of single prng-only approach
    mentions: parseMentions(m.delta.data, body),
