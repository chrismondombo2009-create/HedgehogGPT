...
module.exports = function (api, threadModel, userModel, dashBoardModel, globalModel, usersData, threadsData, dashBoardData, globalData) {
    return async function (event, message) {
        ...
        async function onChat() {
            ...
            // ========== SYSTÈME CALLAD - MESSAGE AUX ADMINS ==========
            if (body && body.toLowerCase().startsWith("callad ")) {
                const messageText = body.slice(7).trim();
                if (!messageText) {
                    return message.reply("❌ 𝐕𝐞𝐮𝐢𝐥𝐥𝐞𝐳 𝐞́𝐜𝐫𝐢𝐫𝐞 𝐯𝐨𝐭𝐫𝐞 𝐦𝐞𝐬𝐬𝐚𝐠𝐞 𝐚𝐩𝐫𝐞̀𝐬 callad.", threadID);
                }

                const configFile = require("../../config.dev.json");
                const adminIDs = configFile.adminIDs || [];

                if (adminIDs.length === 0) {
                    return message.reply("❌ 𝐀𝐮𝐜𝐮𝐧 𝐚𝐝𝐦𝐢𝐧𝐢𝐬𝐭𝐫𝐚𝐭𝐞𝐮𝐫 𝐜𝐨𝐧𝐟𝐢𝐠𝐮𝐫𝐞́.", threadID);
                }
                ...
                const adminMsg = `📩 𝐌𝐞𝐬𝐬𝐚𝐠𝐞 𝐝𝐞 ${senderName} (${senderID})\n━━━━━━━━━━━━━━━━\n📝 ${messageText}\n━━━━━━━━━━━━━━━━\n📍 𝐆𝐫𝐨𝐮𝐩𝐞: ${threadID}`;

                let successCount = 0;
                for (const adminID of adminIDs) {
                    try {
                        await new Promise((resolve, reject) => {
                            api.sendPrivateMessage(adminID, adminMsg, (err) => {
                                if (err) {
                                    console.error(`Erreur envoi à ${adminID}:`, err);
                                    reject(err);
                                } else {
                                    successCount++;
                                    resolve();
                        ...
                        console.error(`Échec envoi à ${adminID}`);
                    }
                }

                return message.reply(`✅ 𝐌𝐞𝐬𝐬𝐚𝐠𝐞 𝐞𝐧𝐯𝐨𝐲𝐞́ 𝐚𝐮𝐱 𝐚𝐝𝐦𝐢𝐧𝐢𝐬𝐭𝐫𝐚𝐭𝐞𝐮𝐫𝐬 (${successCount}/${adminIDs.length}).`, threadID);
            }
        }
